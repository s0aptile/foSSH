...
**The number is a count of proxies, not an on/off switch.** `1` means "there is exactly one hop in front of foSSH that I control." foSSH then reads the entry that hop appended — the rightmost one — rather than the leftmost, which is whatever the original client typed and therefore proves nothing. If you have a CDN in front of that hop too, set `2`, and so on. `FOSSH_TRUST_FORWARDED_FOR=1` still means what it always meant for anyone who already had it set; nothing needs changing. Anything foSSH doesn't recognise, including a typo, reads as `0` and the header is ignored.

Counting from the right is what makes an appending proxy — nginx with `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for`, and most CDNs — genuinely unspoofable: a visitor who sends their own `X-Forwarded-For` only pushes their invention further left, while the entry your proxy wrote stays where it is.

**It does not, on its own, make an arbitrary caller unable to lie, and this is why the "Restricting who can reach the foSSH side of this" step above is not optional.** Anyone who can open a TCP connection to `fossh-cgi` directly *is* the last hop, so whatever they put in the header is what the last hop wrote. Hop counting protects the chain; it cannot tell you the request came through the chain at all. Only restricting who can reach the port does that.

Left unrestricted, the concrete consequence is that `IpFailBucket` (`crates/fossh-ingest/src/ratelimit.rs`) stops throttling write-key guessing. It buckets auth failures by the same client-IP value everything else on this page uses, so a caller sending a fresh made-up address on every request gets a fresh untouched 20-attempt allowance every time and never trips `429` — confirmed directly against the compiled binary: 40 straight wrong-key requests, each with a distinct spoofed `X-Forwarded-For`, all came back `401`, never once `429`. The write key itself remains intact either way; nothing here weakens authentication. What is lost is the availability backstop. Do the restricting step first, or alongside this one — not after "eventually."
