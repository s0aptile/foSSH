...
# binaries (`fossh-fcgi`, `fossh-tui`) — and BoringSSL's own build
...
if [ -n "$openssl_libdir" ] && [ -e "${openssl_libdir}/libcrypto.so" ]; then
    ...
    echo "build-release: warning: could not locate libcrypto.so via pkg-config (openssl-devel installed?) — the --features ...quic build below may fail to link fossh-fcgi/fossh-tui against SQLCipher's OpenSSL backend" >&2
fi
...
# unconditionally building with --features fossh-fcgi/quic,fossh-tui/quic
# (see the comment further down), `boring-sys`'s own separate,
# independent BoringSSL vendoring (pulled in by `quiche` -> `boring`
# for fossh-tui's/fossh-ipc's cmake-driven C++ build) got exercised by
# an ordinary release build for the first time -- and leaked the real
# build machine's username/home path into `fossh-tui` via C++
...
cd "$project_root"
# --features fossh-fcgi/quic,fossh-tui/quic: without this, the §3.4
...
# for, see ADR-0050 Part 3 and fossh-tui/Cargo.toml's own comment),
...
cargo build --release --workspace --features fossh-fcgi/quic,fossh-tui/quic
